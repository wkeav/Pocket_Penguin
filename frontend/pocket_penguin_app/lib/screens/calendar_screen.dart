...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Future<void> _loadEventsForSelectedDay() async {
    ...
    final response = await http.get(Uri.parse('http://127.0.0.1:5000/get_events?date=$dateStr'));
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body) as List<dynamic>;
      setState(() {
        _events[_selectedDate] = data.map((e) => CalendarEvent(
          e['title'],
          e['type'] == 'habit' ? EventType.habit : EventType.todo,
          e['time'],
        )).toList();
      });
    }
      ...
      child: Column(
        children: [
          ...
          ArcticCard(
            ...
            child: Column(
              children: [
                const Text('Calendar', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Color(0xFF0284C7))),
                const SizedBox(height: 8),
                const Text('Track your schedule and progress', style: TextStyle(fontSize: 14, color: Color(0xFF0369A1))),
                const SizedBox(height: 16),
                Row(
                  children: [
                    _buildStatCard('${_getTotalEvents()}', 'Events', Colors.blue),
                    const SizedBox(width: 16),
                    _buildStatCard('${_getCompletedToday()}', 'Completed', Colors.green),
                  ],
                ),
              ],
            ),
              ...
              children: [
                Row(
                  children: [
                    const Icon(Icons.event, color: Colors.blue, size: 20),
                    const SizedBox(width: 8),
                    Text('Events for ${_formatSelectedDate()}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
                  ],
                ),
                const SizedBox(height: 16),
                if (eventsForSelectedDay.isEmpty)
          ...
          ArcticCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Quick Actions', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
                const SizedBox(height: 12),
                Row(
                  children: [
                    _buildActionButton('Add Event', Icons.add_circle, Colors.blue),
                    const SizedBox(width: 8),
                    _buildActionButton('View Week', Icons.view_week, Colors.green),
                  ],
                ),
              ],
            ),
        ...
        onTap: () async {
          if (label == 'Add Event') {
            ...
            if (newEvent != null) {
              final dateStr = "${_selectedDate.year}-${_selectedDate.month.toString().padLeft(2,'0')}-${_selectedDate.day.toString().padLeft(2,'0')}";
              await http.post(
                Uri.parse('http://127.0.0.1:5000/add_event'),
                headers: {'Content-Type': 'application/json'},
                body: jsonEncode({'date': dateStr, 'title': newEvent.title, 'time': newEvent.time, 'type': newEvent.type.name}),
              );
              _loadEventsForSelectedDay();
            }
          }
        ...
        child: Container(
          ...
          child: Column(
            children: [
              Icon(icon, color: color, size: 20),
              const SizedBox(height: 4),
              Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: color), textAlign: TextAlign.center),
            ],
          ),
        ),
      ),
  ...
  Widget _buildCalendarGrid() {
    ...
    return Column(
      children: [
        Row(
          children: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((d) => Expanded(child: Center(child: Text(d, style: const TextStyle(fontWeight: FontWeight.w500, color: Colors.grey, fontSize: 12))))).toList(),
        ),
        const SizedBox(height: 8),
        ...List.generate(6, (weekIndex) {
          return Row(
            children: List.generate(7, (dayIndex) {
              ...
              return Expanded(
                child: GestureDetector(
                  onTap: () { setState(() { _selectedDate = date; _loadEventsForSelectedDay(); }); },
                  child: Container(
                    height: 40,
                    margin: const EdgeInsets.all(2),
                    decoration: BoxDecoration(
                    ...
                    child: Center(
                      child: Text(dayNumber.toString(), style: TextStyle(color: isSelected ? Colors.white : isToday ? Colors.blue[600] : Colors.black, fontWeight: isSelected || isToday ? FontWeight.bold : FontWeight.normal)),
                    ),
                  ),
                ),
              );
  ...
  Widget _buildEventItem(CalendarEvent event) {
    final color = event.type == EventType.habit ? Colors.green : Colors.blue;
    final icon = event.type == EventType.habit ? Icons.check_circle : Icons.task;
    return Container(
      padding: const EdgeInsets.all(12),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(event.title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
  ...
  int _getTotalEvents() => _events.values.fold(0, (total, e) => total + e.length);
  int _getCompletedToday() => 3; // Mock
}
...
extension EventTypeExtension on EventType {
  String get name => this == EventType.habit ? 'habit' : 'todo';
}

class AddEventDialog extends StatefulWidget {
  final DateTime selectedDate;
  const AddEventDialog({required this.selectedDate, super.key});
  @override
...
class _AddEventDialogState extends State<AddEventDialog> {
  ...
  EventType _selectedType = EventType.todo;
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        ...
        TextButton(
          onPressed: () {
            final newEvent = CalendarEvent(_titleController.text, _selectedType, _timeController.text);
